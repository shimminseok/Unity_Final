...
namespace DissolveExample
{
    public class DissolveChilds : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            {
                Material[] dissolveMaterials = new Material[render.materials.Length];

                int index = 0;
                foreach (Material material in render.materials)
                {
                    Texture  originalTexture     = material.GetTexture("_MainTex");
                    Material newDissolveMaterial = new(dissolveMaterial);
                    newDissolveMaterial.SetTexture("_BaseMap", originalTexture);
                    dissolveMaterials[index++] = newDissolveMaterial;
                }

                render.materials = dissolveMaterials;
            }
        }
