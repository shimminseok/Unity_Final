...
{
    // CSV 파일 경로
    static string csvPath = "Assets/CSV/Item/ItemData_KR_v01.csv";
    ...
    private static void ImportCSV(string path)
    {
        ...
        for (int i=1; i<lines.Length; i++)
        {
            ...
            EquipmentItemSO item = ScriptableObject.CreateInstance<EquipmentItemSO>();
            ...
            {
                item.Stats.Add(new StatData {StatType = StatType.AttackPow, ModifierType = StatModifierType.Equipment, Value = attack });
            }
            if (float.TryParse(values[9], out float maxHp))
            {
                item.Stats.Add(new StatData { StatType = StatType.MaxHp, ModifierType = StatModifierType.Equipment, Value = maxHp });
            }
            if (float.TryParse(values[10], out float defense))
            {
                item.Stats.Add(new StatData { StatType = StatType.Defense, ModifierType = StatModifierType.Equipment, Value = defense });
            }
            if (float.TryParse(values[11], out float criticalRate))
            {
                item.Stats.Add(new StatData { StatType = StatType.CriticalRate, ModifierType = StatModifierType.Equipment, Value = criticalRate });
            }
            if (float.TryParse(values[12], out float criticalDamage))
            {
                item.Stats.Add(new StatData { StatType = StatType.CriticalDam, ModifierType = StatModifierType.Equipment, Value = criticalDamage });
            }
            if (float.TryParse(values[13], out float speed))
            {
                item.Stats.Add(new StatData { StatType = StatType.Speed, ModifierType = StatModifierType.Equipment, Value = speed });
            }
            if (float.TryParse(values[14], out float counter))
            {
                item.Stats.Add(new StatData { StatType = StatType.Counter, ModifierType = StatModifierType.Equipment, Value = counter });
            }
