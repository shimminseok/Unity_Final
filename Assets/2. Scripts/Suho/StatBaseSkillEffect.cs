...
public class StatBaseSkillEffect // 개편 필요
{
    ...
    public void AffectTargetWithSkill(IDamageable target) // 실질적으로 영향을 끼치는 부분
    {
         ...
         */
        List<StatusEffectData> skillResults = TranslateFromSkillEffectDataToStatusEffectData();

        foreach (StatusEffectData skillResult in skillResults)
        {
            if (skillResult.Stat.StatType == StatType.CurHp)
            {
                target.TakeDamage(skillResult.Stat.Value); // TakeDamage 자체적으로 방어력으로 인한 데미지 감소 공식 존재하는 가?
            }
            else
            {
                target.Collider.GetComponent<Unit>().StatusEffectManager.ApplyEffect(BuffFactory.CreateBuff(skillResult));
            }
        }
